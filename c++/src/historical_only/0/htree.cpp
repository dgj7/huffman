...
/*******************************************************************************
	HUFFMANTREE
*******************************************************************************/
HuffmanTree::HuffmanTree( stringtype input, DataType howToCompress, TreeType treetype )
{
	if( howToCompress == compressString )
	{
		// compress a string
		if( treetype == compress )
		{
			// iterate through the string and store the frequency data in a vector
			for( size_t n = 0; n < input.size(  ); n++ )
			{
				size_t loc = 0;
				if( p_find_char_in_vector( input[n], loc ) )
				{
			...
			// create a list of trees from the vector of frequency data.
			...
			// put the frequency data into _codes variable
			...
			// print the codes, for the debug version
		...
		else if( treetype == decompress )
		{
			std::cout << "\nSTRING DECOMPRESSION NOT YET IMPLEMENTED!\n";
		}
		else
		{
	...
	else if( howToCompress == compressFile )
	{
		if( treetype == compress )
		{
			DataFormat format;
			...
			infile.open( input.c_str(  ), std::ios::in | std::ios::binary );
			if( !infile )
			{
				std::cout << "\nERROR OPENING FILE!\n";
			}
			...
			{
				infile.read( addr( &format ), sizeof( format ) );
				
				while( !infile.eof(  ) )
				{
					size_t loc = 0;
					if( p_find_char_in_vector( format.c, loc ) )
					{
						_freqdata[loc].second++;
					}
					else
		...
		{
			std::cout << "\nFILE DECOMPRESS NOT YET IMPLEMENTED!\n";
		}
		else
		{
			std::cout << "\nUNRECOGNZIED TREETYPE!\n";
		}
	}
	else
	{
		std::cout << "\nHOW TO COMPRESS-TYPE UNRECOGNIZED!\n";
	}
}

/*
HuffmanTree::HuffmanTree( std::ifstream &infile, TreeType treetype )
{
	if( treetype == compress )
	{
		if( !infile )
	...
	{
		std::cout << "\nFILE DECOMPRESSION NOT YET IMPLEMENTED!\n";
	}
}
*/

void HuffmanTree::p_write_codes( HNode *node, bool preDirection, bool path[], ulint pathLen )
{
	if( node == NULL )
		return;
...
void HuffmanTree::p_debug_print_codes_vector(  ) const
{
	//
}

bool HuffmanTree::p_find_char_in_vector( Data findMe, size_t &location )
{
	std::pair<Data, ulint> currentData;
	location = 0;
	
	// using simple search algorithm for now
	// improve this later!
...
bool HuffmanTree::get_code_for_symbol( Data findMe, std::vector< bool > &retval )
{
...
void HuffmanTree::debug_print_vector(  ) const
{
	std::cout << "\nfrequency data currently stored in the vector:\n";
	for( size_t s = 0; s < _freqdata.size(  ); s++ )
	{
		std::cout << "\t[" << _freqdata[s].first << ", " << _freqdata[s].second << "]\n";
	}
}
